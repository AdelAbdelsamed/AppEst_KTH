...
function run_simulation(app, root, simulationfile)
    ...
    robo_plots = gobjects(1 , n_timesteps);
    ...
    for timestep = 1:n_timesteps
        ...
        if global_localization
            ...
            robo_plots(timestep) = plot(app.SimulationAxis, mu(1), mu(2), 'gx');
        end
    ...
    if stop_execution == 0
        ...
        plot(pose_errors(1,:));
        ylabel('error\_x [m]');
        title(sprintf('error on x, mean error = %.4fm, mean absolute err = %.4fm', mex, maex));
        subplot(3,1,2);
        plot(pose_errors(2,:));
        ylabel('error\_y [m]');
        title(sprintf('error on y, mean error = %.4fm, mean absolute err = %.4fm', mey, maey));
        subplot(3,1,3);
        plot(pose_errors(3,:));
        xlabel('simulation time [s]');
        ylabel('error\_\theta [°]');
        title(sprintf('error on \\theta, mean error = %.4fm, mean absolute err = %.4fm ', met, maet));
        ...
        app.Error_theta_Field.Value = round(maet/pi*360);
